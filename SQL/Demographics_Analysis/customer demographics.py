...
try:
    ...
    importCSV = '''COPY demographic_data(index,fullname, ip_address, region, instore, age, items, amount) FROM '/Volumes/SQL/Demographic_Data_Orig.csv'
    DELIMITER ','
    CSV HEADER;'''
...
# Avg selling price by age group in-store vs non in store
...
'''
Offline sales account for 54% of overall sales, driven by consumers that are 3 years older than the overall average (48 vs 45), but these customers buy products with an average selling price $13 above the overall average. 
...
This is primarily driven by offline sales from 18-29's.
...
in both group 2 and 4 to definitively determine which customers are more valuable/important...and consequently which customers to focus on.
